            ...
            <div class="parent-title" @click="tap(item)">
              <div class="tree-left">
                <div
                  class="tree-qs"
                  :class="
                    item.tb === 0 ? 'same-bg' : item.tb > 0 ? 'top-bg' : ''
                  "
                >
                  <el-icon
                    class="tree-qs-icon"
              ...
              <div class="tree-right">
                <div class="tree-pre" :title="item.total">
                  <div
                    :style="{
                      width:
                        (item.total / total) * 100 < 3
                          ? '3px'
                          : (item.total / total) * 100 + 'px',
                    }"
                ...
                <div
                  ...
                  :title="item.total"
                  :class="
                    item.tb === 0 ? 'same-text' : item.tb > 0 ? 'top-text' : ''
                  "
                >
                  {{ item.total }}
                </div>
      ...
      <div :key="index" v-else style="padding-left: 10px" @click="tap(item)">
        <div class="child-title">
          <div class="tree-left" style="margin-left: 50px">
            <div
              class="tree-qs tree-qs-small"
              :class="item.tb === 0 ? 'same-bg' : item.tb > 0 ? 'top-bg' : ''"
            >
              <el-icon
                class="tree-qs-icon"
                :class="
