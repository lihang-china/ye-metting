...
let clickBA = function (evt, device) {
    ...
    // let container = parseDom('<div class="map-detail" id="map-detail-' + device.guid + '">' + device.deviceName + '<br />' + device.deviceCode + '</div>')[0];
    const url = process.env.VUE_APP_SCVISUALEDITOR_URL + 'runtimes/'+ baParams[device.deviceName] + '/' + getToken();
    console.log(url);
    var name = device.deviceName;                           //网页名称，可为空;
    ...
    // let container = parseDom('<iframe src="' + url + '" frameborder="0" width="500px" height="500px"></iframe>')[0];
    // let overlay = new Overlay({
    //     id: "tip_overlay_" + device.guid,
    //     //设置弹出框的容器
    //     element: container,
