 <template>
    <div class="dashboard-view-analyse">
        ...
        <div class="page-right-view">
            <div class="right-top">
                <div class="title">{{currentType.label}}{{currentAds.label}}用能趋势分析</div>
                    ...
                    <div class="zt-block">
                        ...
                        <div>
                            ...
                            <div class="qs-label-bottom">
                                <div class="bottom-text" :class="totalDayTB===0?'same-text':totalDayTB>0?'top-text':''">{{date}}年</div>
                                <div>{{area?area.name:'所有部门'}} 白天总用{{currentType.label}}量</div>
                            </div>
                        </div>
                    </div>
                    <div class="zt-block">
                        ...
                        <div>
                            ...
                            <div class="qs-label-bottom">
                                <div class="bottom-text" :class="totalNightTB===0?'same-text':totalNightTB>0?'top-text':''">{{date}}年</div>
                                <div>{{area?area.name:'所有部门'}} 夜间总用{{currentType.label}}量</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="right-bottom">
                <div class="bottom-left">
                    <div class="btn-block">
                        <div :key="item.val" v-for="item in ads"  class="btn" :class="{'active':currentAds.val===item.val}" @click="currentAds=item">{{ item.label }}</div>
                    </div>
                    ...
                    <div class="result-block">
                        <div class="echarts">
                            ...
                            <div class="echarts-content">
                                <line-chart style="width:100%;height:calc(100% + 25px);margin-top: -30px;" :c-data="chartData" v-if="chartData" @tap="chartTap"/>
                            </div>
                        </div>
                        <div class="table">
                            <div class="table-title">
                                <div>日期</div>
                                <div>当前用{{currentType.label}}量（{{currentType.unit}}）</div>
                                <div>同期用{{currentType.label}}量（{{currentType.unit}}）</div>
                                <div>同比涨幅(%)</div>
                            </div>
...
<script>
import { energyByAreaAndDept,energyByAreaAndDeptRit, energyDayByAreaAndDeptRit, energyHourByAreaAndDeptRit } from '@/api/energy/app/analyse';
...
export default {
    ...
    methods:{
            ...
            energyByAreaAndDeptRit(this.currentAds.val,query).then(res => {
                ...
                if(d.detailList){
                    this.detailList = d.detailList.map(de => ({
                        ...
                        percent: de.percent?(de.percent*100).toFixed(2):0.00,
                    }))
        ...
        getEnergyTreeRitDay(date){ // 获取每日明细
            var query = this.getQuery()
            query.date = date
            this.dateDay = date
        ...
        getEnergyTreeRitHour(date){ // 获取每小时明细
            var query = this.getQuery()
