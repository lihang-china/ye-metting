 <template>
  <div class="dashboard-view-analyse">
    ...
    <div class="page-right-view">
      <div class="right-top">
        <div class="title">{{ currentType.label }}每周用能趋势分析</div>
          ...
          <div class="result-block">
            <div class="echarts">
              <div class="echarts-title">
                {{ dateHtml }}周用{{ currentType.label }}量统计分析
              </div>
            ...
            <div class="echarts" style="margin-top: 15px">
              <div class="echarts-title">
                {{ dayDateHtml }}用{{ currentType.label }}量统计分析
              </div>
              <div class="echarts-content">
                <bar-chart
                  style="
...
export default {
  ...
  methods: {
    ...
    getEnergyTree() {
      ...
      energyByWeek(this.currentAds.val, query).then((res) => {
        var result = null;
    ...
    getEnergyTreeRit() {
      // 获取右侧数据
      var query = this.getQuery();
      energyByWeekRit(this.currentAds.val, query).then((res) => {
        var d = res.data;
        this.chartData = null;
        ...
        if (d.detailList) {
          this.detailList = d.detailList.map((de) => ({
            date: de.date,
            weekendTotal: de.weekendTotal ? de.weekendTotal : 0,
            workTotal: de.workTotal ? de.workTotal : 0,
          }));
            ...
            series: [
              {
                name: "周末用" + text + "量",
                type: "bar",
                data: this.detailList.map((i) => i.weekendTotal),
              },
              {
                name: "非周末用" + text + "量",
                type: "bar",
                data: this.detailList.map((i) => i.workTotal),
              },
            ],
          };
        }
        ...
        if (this.detailList && this.detailList.length > 0) {
          this.getWeekDate(this.detailList[0].date);
        } else {
          this.dayChartData = null;
        }
      });
    },
    getEnergyDayByWeekRit() {
      // 获取右侧数据
      var query = this.getQuery();
      query.weekName = this.dayDateHtml;
      energyDayByWeekRit(this.currentAds.val, query).then((res) => {
        var d = res.data;
        this.dayChartData = null;
        if (d) {
          var dataList = d.map((de) => ({
            date: de.dayOfWeekName,
            totalValue: de.totalValue ? de.totalValue : 0,
          }));

          var text = this.currentType.label;
          this.dayChartData = {
            xAxisData: dataList.map((i) => i.date),
            series: [
              {
                type: "bar",
                data: dataList.map((i) => i.totalValue),
              },
            ],
          };
        }
    ...
    getWeekDate(str) {
      this.dayDateHtml = str;
      // var arr = str.match(/\d+(.\d+)?/g);
      // this.dateDay = arr[1]
      this.getEnergyDayByWeekRit();
    },
    getItemRoot() {
      // 获取分项数据
