...
<script>
import { getDept } from "@/api/energy/basic/dept";
import {
  listDeptConfigSetting,
  getDeptConfig,
  delDeptConfig,
  addDeptConfig,
} from "@/api/energy/basic/deptConfig";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listDeptConfigSetting(this.queryParams).then((response) => {
    ...
    handleRemove(row) {
      ...
      this.$confirm("是否确定移除所选择的设备?", "警告", {
        ...
        .then(() => {
          ...
          delDeptConfig(ids).then((response) => {
            loading.close();
