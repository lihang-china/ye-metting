...
<script>
import { monitorByItem, monitorByItemRit } from "@/api/energy/app/monitor";
...
export default {
  ...
  data() {
    return {
      ...
      ads: [{ label: "分项", val: "item" }],
      currentType: { label: "电", val: 1, unit: "kWh" },
      currentAds: { label: "分项", val: "item" },
  ...
  methods: {
    ...
    getQuery() {
      return {
        itemCode: this.area ? this.area.id : null,
    ...
    getEnergyTreeRit() {
      // 获取右侧数据
      var query = this.getQuery();
      monitorByItemRit(query).then((res) => {
        var d = res.data;
        this.chartData = null;
