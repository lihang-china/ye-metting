  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
      <el-form-item label="模板名称">
        <el-input v-model="templet.className" :disabled="true" size="small" />
      </el-form-item>
      <el-form-item label="模板编码">
        <el-input v-model="templet.classCode" :disabled="true" size="small" />
      </el-form-item>
...
import { getClass } from "@/api/equipment/class";
...
export default {
  ...
  created() {
    ...
    getClass(templetId).then((response) => {
      this.templet = response.data;
      this.queryParams.templetId = response.data.classId;
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      const templetId = this.templet.classId;
      this.$confirm(
    ...
    submitForm: function () {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          this.form.templetId = this.templet.classId;
