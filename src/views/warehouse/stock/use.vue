    ...
    <el-row :gutter="20">
      ...
      <el-col :span="20" :xs="24">
        <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="98px">
          <el-form-item label="库房" prop="roomId">
            <el-select
        ...
        <el-table v-loading="loading" :data="spList" style="width: 100%">
          ...
          <el-table-column label="当前拥有量" align="center" prop="quantity" width="200" />
        ...
        <el-dialog :title="title" :visible.sync="open" width="1500px" :close-on-click-modal="false">
          <el-dialog width="70%" title="查看记录" :visible.sync="openOperate" append-to-body>
...
export default {
  ...
  methods: {
    loadNode(node, resolve) {
      if (node.data.id) {
        getSearchListUser(node.data.id).then((response) => {
    ...
    handleNodeClick(data) {
      ...
      if (data.value) {
        this.nickName = data.label;
        this.queryParams.useBy = data.value;
      } else {
        this.queryParams.useBy = 0;
      }
      this.getList();
    },
    //获取部门列表
    getDeptList() {
      deptTreeselect().then((response) => {
        let depts = response.data[0].children;
        this.options = depts;
      });
    },
    // 获取仓库
    ...
    // 重置
    resetQuery() {
      this.queryParams.useBy = undefined;
    ...
    handleView(row) {
      ...
      getRecordUse(params).then((response) => {
