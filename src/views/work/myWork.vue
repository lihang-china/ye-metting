...
<script>
import { myWorkList, entrustWork } from "@/api/work/work";
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true;
      myWorkList(this.queryParams).then((response) => {
    ...
    handleUser(value) {
      //console.log(value);
      const loading = this.openLoading("正在委托任务,请稍后......");
      entrustWork(this.taskId, { userName: value.userIds })
        .then((response) => {
          loading.close();
          if (response.code === 200) {
            this.msgSuccess("委托成功");
            this.getList();
          } else {
            this.msgError("委托成功");
          }
